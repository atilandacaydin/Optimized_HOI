...
def mi_gg(x, y, biascorrect=True, demeaned=False):
    ...
    Cxy /= float(Ntrl-1.)                                               #implementation of einsum func
    ...
    HX = np.log(np.einsum('...ii->...i', chCx)).sum(-1) # + 0.5*Nvarx*(np.log(2*np.pi)+1.0)
    HY = np.log(np.einsum('...ii->...i', chCy)).sum(-1) # + 0.5*Nvary*(np.log(2*np.pi)+1.0)
    HXY = np.log(np.einsum('...ii->...i', chCxy)).sum(-1) # + 0.5*Nvarxy*(np.log(2*np.pi)+1.0)
...
def mi_model_gd(x, y, Ym, biascorrect=True, demeaned=False):
    ...
    for yi in range(Ym):
        ...
        Cm = np.einsum('...ij, ...kj->...ik', xm, xm) / float(Ntrl_y[yi]-1)
        chCm = np.linalg.cholesky(Cm)
        Hcond[yi] = np.log(np.einsum('...ii->...i', chCm)).sum(-1) # + c*Nvarx
    ...
    Hunc = np.log(np.einsum('...ii->...i', chC)).sum(-1) # + c*Nvarx
...
def gccmi_ccd(x,y,z,Zm):
    ...
    # calculate gcmi for each z value ### list comprehension
    Icond = np.zeros(Zm)
    Pz = np.zeros(Zm)
    for zi in range(Zm):
        idx = z==zi
